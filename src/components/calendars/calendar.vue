<template>
 ...
 kakao(){
    ...
    this.$axios.get("http://localhost:8181/api/kakao/member", {headers:{'token':token}})
    .then((res)=>{
      // 가입이 되었는지 / 1번 카카오톡 가입자인지 까지 확인
      console.log(res.data)
      if(res.data.id == 1){
      // 1번 카카오 가입자이면 토큰값으로 톡캘린더 권한이 있는지 확인
      this.$axios.get("http://localhost:8181/api/kakao/tokencheck", {headers:{'token':token}})
      .then((res)=>{
        // result가 false 이면 권한이 없는것
        console.log(res.data)
      ...
      const redirectURI = encodeURIComponent("http://localhost:8181/api/kakao/token")
      const kakaoAuthURL = 'https://kauth.kakao.com/oauth/authorize?client_id='+clientId+"&redirect_uri="+redirectURI+"&response_type=code"+"&scope=talk_calendar"
      if(result == false){
